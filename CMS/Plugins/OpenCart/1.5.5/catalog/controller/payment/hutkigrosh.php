...
//-----------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if (!$res) {
            echo $this->getError();
            $this->apiLogOut(); // Завершаем сеанс
            exit;
        }
...
//
        ...
        $this->_billID = $this->apiBillNew($data);
        if (!$this->_billID) {
            echo $this->error;
            echo $this->getError();
            $this->apiLogOut(); // Завершаем сеанс
        ...
        echo '<hr>';
        print_r($this->apiBgpbPay($dataBgpb));
        ?>
        <br>
        <hr>
        <div class="alfaclick">
	...
	public function notify() {
		if(isset($hg_data["purchaseid"])) {
            ...
            $info = $this->apiBillInfo($hg_data["purchaseid"]);
            if (!$info) {
                echo $this->getError();
                $this->apiLogOut(); // Завершаем сеанс
                exit;
     ...
     */
    public function constructMy($is_test)
    {
        $this->test = $is_test;
        if ($is_test) {
